...
private extension MediaPickerController {
    ...
    func addChooseExistingMediaActionToSheet(actionSheet: UIAlertController) {
        let chooseExistingAction = UIAlertAction(title: Strings.ChoosePhoto, style: UIAlertActionStyle.Default) { (_) -> Void in
    ...
    struct Strings {
        ...
        static let ChoosePhoto = NSLocalizedString("Choose existing photo or video", comment: "Text for an option that lets the user choose an existing photo or video in a generic action sheet for picking media from the device.")
